...
# ~~ajout du dossier ocr_runner.py au path python
...
app = Flask(__name__)
CORS(app)  # ~~autorise les requetes depuis le front vite (localhost:5173)
...
def upload():
    # ~~verification presence du fichier dans la requete
    if "file" not in request.files:
        return jsonify({"error": "Aucun fichier recu (cle attendue : 'file')."}), 400

    file = request.files["file"]

    # ~~rejet si nom de fichier vide
    if file.filename == "":
        return jsonify({"error": "Nom de fichier vide."}), 400

    # ~~rejet si format non supporte
    ...
    try:
        # ~~choix de l'extracteur selon le type de fichier
        if suffix == ".pdf":
